      ...
      <div className="bg-white rounded-3xl max-w-md w-full max-h-[90vh] overflow-y-auto border border-stone-200 shadow-2xl">
        <div className="p-6">
          <div className="flex justify-between items-start mb-6">
            <div>
              <span className={`${typeColors[wine.type]} text-white text-xs font-medium px-3 py-1 rounded-full`}>
                {wine.type}
              </span>
              <h2 className="text-2xl font-bold text-stone-800 mt-3">{wine.name}</h2>
              <p className="text-stone-500">{wine.year}</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="space-y-4 mb-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-stone-100 rounded-xl p-3">
                ...
                <p className="text-stone-800 font-medium">{wine.country || '-'}</p>
              </div>
              <div className="bg-stone-100 rounded-xl p-3">
                <p className="text-stone-500 text-xs">Regio</p>
                <p className="text-stone-800 font-medium">{wine.region || '-'}</p>
              </div>
              <div className="bg-stone-100 rounded-xl p-3">
                <p className="text-stone-500 text-xs">Druif</p>
                <p className="text-stone-800 font-medium">{wine.grapes || '-'}</p>
              </div>
              <div className="bg-stone-100 rounded-xl p-3">
                <p className="text-stone-500 text-xs">Beste op dronk</p>
                <p className="text-stone-800 font-medium">{wine.bestBefore || '-'}</p>
              </div>
            </div>
            ...
            {wine.tasteProfile && (
              <div className="bg-stone-100 rounded-xl p-4">
                ...
                <p className="text-stone-800">{wine.tasteProfile}</p>
              </div>
            )}
            ...
            {wine.pairingAdvice && (
              <div className="bg-stone-100 rounded-xl p-4">
                ...
                <p className="text-stone-800">{wine.pairingAdvice}</p>
              </div>
            )}
          </div>
